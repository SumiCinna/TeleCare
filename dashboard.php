...
<body class="bg-dark text-white font-sans antialiased flex">

<aside class="w-64 shrink-0 h-screen sticky top-0 bg-dark-sidebar border-r border-white/5 flex flex-col">
    <div class="h-16 flex items-center px-6 border-b border-white/5">
        <div class="flex items-center gap-2">
            ...
            <span class="font-bold text-sm">Tele<span class="text-primary">Care</span> <span class="text-white/30 text-xs">AI</span></span>
        </div>
    </div>
    ...
    <nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
        ...
        $navItems = [
            ...
            ['label' => 'Consultations', 'icon' => 'M15 10l4.553-2.069A1 1 0 0121 8.87v6.26a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z'],
            ['label' => 'Documents',     'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
            ['label' => 'Meetings',      'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],
        ];
        if ($userRole === 'admin') {
            $navItems[] = ['label' => 'Users', 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'];
        }
        $navItems[] = ['label' => 'Settings', 'icon' => 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z'];
        foreach ($navItems as $item):
            $active = $item['active'] ?? false;
        ?>
        <a href="#" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all <?= $active ? 'bg-primary/10 text-primary font-medium' : 'text-white/50 hover:text-white hover:bg-white/5' ?>">
    ...
    <div class="p-4 border-t border-white/5">
        ...
        <button onclick="openLogoutModal()" class="flex items-center gap-2 text-xs text-white/40 hover:text-red-400 transition-colors px-1">
...
<main class="flex-1 min-h-screen overflow-y-auto">
    <header class="h-16 flex items-center justify-between px-8 border-b border-white/5 bg-dark/50 sticky top-0 backdrop-blur-md z-10">
        <div>
...
<div id="logoutModal" class="fixed inset-0 z-50 hidden items-center justify-center">
    ...
    <div class="relative bg-dark-card border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
        ...
        <div class="flex gap-3">
            ...
            <a href="auth/logout.php" class="flex-1 bg-red-500 hover:bg-red-600 text-white font-medium py-2.5 rounded-xl text-sm transition-all text-center">
                Yes, Sign Out
            </a>
        </div>
    </div>
